...
export default function CalendarView({ projectId }: { projectId?: string }) {
  ...
  const [month, setMonth] = useState(new Date()); // mes actual
  ...
  return (
    <div>
      <div className="mb-3 flex items-center gap-2">
        <button className="border rounded px-2 py-1" onClick={()=>setMonth(new Date(month.getFullYear(), month.getMonth()-1, 1))}>←</button>
      ...
      <div className="grid grid-cols-7 gap-2">
        ...
        {days.map((d,i)=> {
          ...
          return (
            <div key={i} className={`border rounded p-2 min-h-24 ${isOtherMonth ? "bg-gray-50" : "bg-white"}`}>
              ...
              <div className="space-y-1">
                ...
                {dayTasks.length > 4 && (
                  <div className="text-[11px] text-gray-500">+{dayTasks.length-4} más</div>
                )}
              </div>
            </div>
          );
